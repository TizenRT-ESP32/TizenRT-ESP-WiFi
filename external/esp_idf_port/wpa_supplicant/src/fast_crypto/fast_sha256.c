 ...
 */
void fast_hmac_sha256_vector(const uint8_t *key, size_t key_len, size_t num_elem, const uint8_t *addr[], const size_t *len, uint8_t *mac)
{
 ...
 */
void fast_hmac_sha256(const uint8_t *key, size_t key_len, const uint8_t *data, size_t data_len, uint8_t *mac)
{
 ...
 * given key.
 */
void fast_sha256_prf(const uint8_t *key, size_t key_len, const char *label, const uint8_t *data, size_t data_len, uint8_t *buf, size_t buf_len)
{
	uint16_t counter = 1;
	size_t pos, plen;
	...
	while (pos < buf_len) {
		...
		if (plen >= SHA256_MAC_LEN) {
			fast_hmac_sha256_vector(key, key_len, 4, addr, len, &buf[pos]);
			pos += SHA256_MAC_LEN;
		} else {
