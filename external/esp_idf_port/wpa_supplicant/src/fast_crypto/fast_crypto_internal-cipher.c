...
int fast_crypto_cipher_encrypt(struct crypto_cipher *ctx, const uint8_t *plain, uint8_t *crypt, size_t len)
{
	size_t i, j, blocks;
	...
	case CRYPTO_CIPHER_ALG_RC4:
		...
		rc4_skip(fast_ctx->u.rc4.key, fast_ctx->u.rc4.keylen, fast_ctx->u.rc4.used_bytes, crypt, len);
	...
	case CRYPTO_CIPHER_ALG_3DES:
		...
		for (i = 0; i < blocks; i++) {
			...
			des3_encrypt(fast_ctx->u.des3.cbc, &fast_ctx->u.des3.key, fast_ctx->u.des3.cbc);
	...
	case CRYPTO_CIPHER_ALG_DES:
		...
		for (i = 0; i < blocks; i++) {
			...
			des_block_encrypt(fast_ctx->u.des.cbc, fast_ctx->u.des.ek, fast_ctx->u.des.cbc);
...
int fast_crypto_cipher_decrypt(struct crypto_cipher *ctx, const uint8_t *crypt, uint8_t *plain, size_t len)
{
	size_t i, j, blocks;
	uint8_t tmp[32];
	struct fast_crypto_cipher *fast_ctx;
	...
	case CRYPTO_CIPHER_ALG_RC4:
		...
		rc4_skip(fast_ctx->u.rc4.key, fast_ctx->u.rc4.keylen, fast_ctx->u.rc4.used_bytes, plain, len);
		fast_ctx->u.rc4.used_bytes += len;
		break;
	case CRYPTO_CIPHER_ALG_AES:
		if (len % AES_BLOCK_SIZE) {
