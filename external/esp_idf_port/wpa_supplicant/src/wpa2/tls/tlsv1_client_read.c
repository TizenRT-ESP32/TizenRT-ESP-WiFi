...
static int tlsv1_process_diffie_hellman(struct tlsv1_client *conn, const u8 *buf, size_t len)
{
	...
	if (conn->dh_p_len == 0 || end - pos < (int)conn->dh_p_len) {
		wpa_printf(MSG_DEBUG, "TLSv1: Invalid dh_p length %lu", (unsigned long)conn->dh_p_len);
		goto fail;
	}
...
static int tls_process_server_finished(struct tlsv1_client *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
	...
	if (conn->rl.tls_version >= TLS_VERSION_1_2) {
		...
		if (wpa2_crypto_funcs.crypto_hash_finish) {
			if (conn->verify.sha256_server == NULL || wpa_crypto_funcs.crypto_hash_finish(conn->verify.sha256_server, hash, &hlen) < 0) {
				tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
