...
static int tls_write_server_key_exchange(struct tlsv1_server *conn, u8 **msgpos, u8 *end)
{
	...
	if (os_memcmp(conn->dh_secret, conn->cred->dh_p, conn->dh_secret_len) > 0) {
	...
	if (wpa2_crypto_funcs.crypto_mod_exp) {
		if (wpa2_crypto_funcs.crypto_mod_exp(conn->cred->dh_g, conn->cred->dh_g_len, conn->dh_secret, conn->dh_secret_len, conn->cred->dh_p, conn->cred->dh_p_len, dh_ys, &dh_ys_len)) {
			tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
			os_free(dh_ys);
			return -1;
		}
...
static int tls_write_server_finished(struct tlsv1_server *conn, u8 **msgpos, u8 *end)
{
	...
	if (conn->rl.tls_version >= TLS_VERSION_1_2) {
		...
		if (wpa2_crypto_funcs.crypto_hash_finish) {
			if (conn->verify.sha256_server == NULL || wpa2_crypto_funcs.crypto_hash_finish(conn->verify.sha256_server, hash, &hlen)
				< 0) {
