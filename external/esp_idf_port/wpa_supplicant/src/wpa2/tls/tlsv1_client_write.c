...
static int tlsv1_key_x_anon_dh(struct tlsv1_client *conn, u8 **pos, u8 *end)
{
	...
	if (wpa2_crypto_funcs.crypto_mod_exp) {
		if (wpa2_crypto_funcs.crypto_mod_exp(conn->dh_g, conn->dh_g_len, csecret_start, csecret_len, conn->dh_p, conn->dh_p_len, dh_yc, &dh_yc_len)) {
			tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
			os_free(csecret);
			os_free(dh_yc);
			return -1;
	...
	*pos += dh_yc_len;
	...
	if (wpa2_crypto_funcs.crypto_mod_exp) {
		if (wpa2_crypto_funcs.crypto_mod_exp(conn->dh_ys, conn->dh_ys_len, csecret_start, csecret_len, conn->dh_p, conn->dh_p_len, shared, &shared_len)) {
			tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
			os_free(csecret);
			os_free(shared);
			return -1;
...
static int tls_write_client_finished(struct tlsv1_client *conn, u8 **msgpos, u8 *end)
{
	...
	if (conn->rl.tls_version >= TLS_VERSION_1_2) {
		hlen = SHA256_MAC_LEN;
		if (wpa2_crypto_funcs.crypto_hash_finish) {
			if (conn->verify.sha256_client == NULL || wpa2_crypto_funcs.crypto_hash_finish(conn->verify.sha256_client, hash, &hlen)
				< 0) {
				tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
