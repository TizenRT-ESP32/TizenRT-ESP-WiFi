...
void x509_name_string(struct x509_name *name, char *buf, size_t len)
{
	...
	for (i = 0; i < name->num_attr; i++) {
		...
		ret = sprintf(pos, "%s=%s, ", x509_name_attr_str(name->attr[i].type), name->attr[i].value);
	...
	if (name->email) {
		...
		ret = sprintf(pos, "/emailAddress=%s", name->email);
		if (ret < 0 || ret >= end - pos) {
			goto done;
		}
		pos += ret;
...
static int x509_parse_validity(const u8 *buf, size_t len, struct x509_certificate *cert, const u8 **next)
{
	...
	*next = pos + plen;
	...
	wpa_printf(MSG_DEBUG, "X509: Validity: notBefore: %lu notAfter: %lu", (unsigned long)cert->not_before, (unsigned long)cert->not_after);
...
skip_digest_oid:
	...
	case 11:					/* sha256WithRSAEncryption */
		if (wpa2_crypto_funcs.sha256_vector) {
			wpa2_crypto_funcs.sha256_vector(1, &cert->tbs_cert_start, (int *)&cert->tbs_cert_len, hash);
		} else {
	...
	if (hdr.length != hash_len || os_memcmp(hdr.payload, hash, hdr.length) != 0) {
