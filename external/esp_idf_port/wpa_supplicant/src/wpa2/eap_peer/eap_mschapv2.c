...
static void eap_mschapv2_password_changed(struct eap_sm *sm, struct eap_mschapv2_data *data)
{
	...
	if (config && config->new_password) {
		...
		if (config->flags & EAP_CONFIG_FLAGS_EXT_PASSWORD) {
			...
			if (config->password) {
				nt_password_hash(config->new_password, config->new_password_len, config->password);
			}
...
static int eap_mschapv2_failure_txt(struct eap_sm *sm, struct eap_mschapv2_data *data, char *txt)
{
	...
	if (pos && os_strncmp(pos, "C=", 2) == 0) {
		...
		if (hex_len == PASSWD_CHANGE_CHAL_LEN * 2) {
			...
			wpa_printf(MSG_ERROR, "EAP-MSCHAPV2: required challenge field " "was not present in failure message\n");
		}
	}
...
#if 0
	wpa_printf(MSG_WARNING, "EAP-MSCHAPV2: failure message: '%s' (retry %sallowed, error %d)", msg, retry == 1 ? "" : "not ", data->prev_error);
#endif
	if (data->prev_error == ERROR_PASSWD_EXPIRED && data->passwd_change_version == 3 && config) {
		if (config->new_password == NULL) {
			wpa_printf(MSG_DEBUG, "EAP-MSCHAPV2: Password expired - " "password change reqired\n");
...
static struct wpabuf *eap_mschapv2_change_password(struct eap_sm *sm, struct eap_mschapv2_data *data, struct eap_method_ret *ret, const struct eap_mschapv2_hdr *req, u8 id)
{
	...
	if (pwhash) {
		...
		old_nt_password_hash_encrypted_with_new_nt_password_hash(new_password, new_password_len, password, password_len, cp->encr_hash);
	}
...
static int eap_mschapv2_check_config(struct eap_sm *sm)
{
	...
	if (config->identity == NULL || config->identity_len == 0) {
		wpa_printf(MSG_ERROR, "EAP-MSCHAPV2: idetity not configured\n");
		return -1;
	}

	if (config->password == NULL || config->password_len == 0) {
...
static int eap_mschapv2_check_mslen(struct eap_sm *sm, size_t len, const struct eap_mschapv2_hdr *ms)
{
	...
	if (sm->workaround) {
		wpa_printf(MSG_DEBUG, "EAP-MSCHAPV2: Workaround, ignore Invalid" " header len=%lu ms_len=%lu\n", (unsigned long)len, (unsigned long)ms_len);
		return 0;
	}
...
int eap_peer_mschapv2_register(void)
{
	...
	eap = eap_peer_method_alloc(EAP_VENDOR_IETF, EAP_TYPE_MSCHAPV2, "MSCHAPV2");
