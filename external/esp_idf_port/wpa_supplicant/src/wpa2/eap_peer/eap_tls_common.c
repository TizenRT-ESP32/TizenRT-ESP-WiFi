...
static int eap_tls_params_from_conf(struct eap_sm *sm, struct eap_ssl_data *data, struct tls_connection_params *params, struct eap_peer_config *config)
{
	 ...
	 * file as-is.
	 */
	if (eap_tls_check_blob(sm, &params->ca_cert, &params->ca_cert_blob, &params->ca_cert_blob_len) || eap_tls_check_blob(sm, &params->client_cert, &params->client_cert_blob, &params->client_cert_blob_len) || eap_tls_check_blob(sm, &params->private_key, &params->private_key_blob, &params->private_key_blob_len)) {
...
int eap_peer_tls_ssl_init(struct eap_sm *sm, struct eap_ssl_data *data, struct eap_peer_config *config, u8 eap_type)
{
	...
	if (config->phase1 && os_strstr(config->phase1, "include_tls_length=1")) {
		wpa_printf(MSG_INFO, "TLS: Include TLS Message Length in " "unfragmented packets");
...
u8 *eap_peer_tls_derive_key(struct eap_sm *sm, struct eap_ssl_data *data, const char *label, size_t len)
{
	...
	if (keys.client_random == NULL || keys.server_random == NULL || keys.master_key == NULL) {
		goto fail;
	}
	...
	os_memcpy(rnd + keys.client_random_len, keys.server_random, keys.server_random_len);

	if (tls_prf_sha1_md5(keys.master_key, keys.master_key_len, label, rnd, keys.client_random_len + keys.server_random_len, out, len)) {
		goto fail;
	}
...
static int eap_peer_tls_reassemble_fragment(struct eap_ssl_data *data, const struct wpabuf *in_data)
{
	...
	if (data->tls_in_left > 0) {
		wpa_printf(MSG_INFO, "SSL: Need %lu bytes more input " "data", (unsigned long)data->tls_in_left);
		return 1;
	}
...
int eap_peer_tls_status(struct eap_sm *sm, struct eap_ssl_data *data, char *buf, size_t buflen, int verbose)
{
	...
	if (tls_get_cipher(data->ssl_ctx, data->conn, name, sizeof(name)) == 0) {
		...
		ret = sprintf(buf + len, "EAP TLS cipher=%s\n", name);
...
int eap_peer_select_phase2_methods(struct eap_peer_config *config, const char *prefix, struct eap_method_type **types, size_t *num_types)
{
	...
	while (start && *start != '\0') {
		...
		if (vendor == EAP_VENDOR_IETF && method == EAP_TYPE_NONE) {
			wpa_printf(MSG_INFO, "TLS: Unsupported Phase2 EAP " "method '%s'\n", start);
		} else {
			num_methods++;
			_methods = (struct eap_method_type *)os_realloc(methods, num_methods * sizeof(*methods));
			if (_methods == NULL) {
				os_free(methods);
				os_free(buf);
				return -1;
