...
static void *eap_tls_init(struct eap_sm *sm)
{
	...
	if (config == NULL || config->private_key == 0) {
...
static struct wpabuf *eap_tls_process(struct eap_sm *sm, void *priv, struct eap_method_ret *ret, const struct wpabuf *reqData)
{
	...
	resp = NULL;
	res = eap_peer_tls_process_helper(sm, &data->ssl, data->eap_type, 0, id, pos, left, &resp);
...
int eap_peer_tls_register(void)
{
	...
	eap = eap_peer_method_alloc(EAP_VENDOR_IETF, EAP_TYPE_TLS, "TLS");
