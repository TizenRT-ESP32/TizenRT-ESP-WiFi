...
static int wps_process_cred_network_key(struct wps_credential *cred, const u8 *key, size_t key_len)
{
	if (key == NULL) {
		...
		if (cred->auth_type == WPS_WIFI_AUTH_OPEN && cred->encr_type == WPS_ENCR_NONE) {
...
static int wps_process_cred_eap_identity(struct wps_credential *cred, const u8 *identity, size_t identity_len)
{
	...
	wpa_hexdump_ascii(MSG_DEBUG, "WPS: EAP Identity", identity, identity_len);
...
static int wps_process_cred_ap_channel(struct wps_credential *cred, const u8 *ap_channel)
{
	if (ap_channel == NULL) {
		return 0;    /* optional attribute */
	}
