...
static void wps_registrar_expire_pins(struct wps_registrar *reg)
{
	...
	dl_list_for_each_safe(pin, prev, &reg->pins, struct wps_uuid_pin, list) {
		if ((pin->flags & PIN_EXPIRES) && os_time_before(&pin->expiration, &now)) {
...
static int wps_registrar_invalidate_wildcard_pin(struct wps_registrar *reg, const u8 *dev_pw, size_t dev_pw_len)
{
	...
	dl_list_for_each_safe(pin, prev, &reg->pins, struct wps_uuid_pin, list) {
		if (dev_pw && pin->pin && (dev_pw_len != pin->pin_len || os_memcmp(dev_pw, pin->pin, dev_pw_len) != 0)) {
...
static int wps_set_ie(struct wps_registrar *reg)
{
	...
	if (wps_build_version(probe) || wps_build_wps_state(reg->wps, probe) || wps_build_ap_setup_locked(reg->wps, probe) || wps_build_selected_registrar(reg, probe) || wps_build_sel_reg_dev_password_id(reg, probe) || wps_build_sel_reg_config_methods(reg, probe) || wps_build_resp_type(probe, reg->wps->ap ? WPS_RESP_AP : WPS_RESP_REGISTRAR) || wps_build_uuid_e(probe, reg->wps->uuid) || wps_build_device_attrs(&reg->wps->dev, probe) || wps_build_probe_config_methods(reg, probe) || (reg->dualband && wps_build_rf_bands(&reg->wps->dev, probe)) || wps_build_wfa_ext(probe, 0, auth_macs, count) || wps_build_vendor_ext(&reg->wps->dev, probe)) {
		wpabuf_free(beacon);
		wpabuf_free(probe);
		return -1;
	}
...
static int wps_get_dev_password(struct wps_data *wps)
{
	...
	if (pin == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No Device Password available for " "the Enrollee");
...
{
	if (wps_build_cred_network_idx(msg, cred) || wps_build_cred_ssid(msg, cred) || wps_build_cred_auth_type(msg, cred) || wps_build_cred_encr_type(msg, cred) || wps_build_cred_network_key(msg, cred) || wps_build_cred_mac_addr(msg, cred)) {
		return -1;
	}
	return 0;
}
...
static enum wps_process_res wps_process_m1(struct wps_data *wps, struct wps_parse_attr *attr)
{
	...
	if (wps->dev_pw_id < 0x10 && wps->dev_pw_id != DEV_PW_DEFAULT && wps->dev_pw_id != DEV_PW_USER_SPECIFIED && wps->dev_pw_id != DEV_PW_MACHINE_SPECIFIED && wps->dev_pw_id != DEV_PW_REGISTRAR_SPECIFIED && (wps->dev_pw_id != DEV_PW_PUSHBUTTON || !wps->wps->registrar->pbc)) {
	...
	if (wps->dev_pw_id >= 0x10) {
		...
		if (token) {
			...
			if (os_memcmp(hash, wps->nfc_pw_token->pubkey_hash, WPS_OOB_PUBKEY_HASH_LEN) != 0) {
	...
	if (wps->dev_pw_id == DEV_PW_PUSHBUTTON) {
		if ((wps->wps->registrar->force_pbc_overlap || wps_registrar_pbc_overlap(wps->wps->registrar, wps->mac_addr_e, wps->uuid_e) || !wps_registrar_p2p_dev_addr_match(wps)) && !wps_registrar_skip_overlap(wps)) {
			...
			wps_fail_event(wps->wps, WPS_M1, WPS_CFG_MULTIPLE_PBC_DETECTED, WPS_EI_NO_ERROR);
...
static enum wps_process_res wps_process_wsc_done(struct wps_data *wps, const struct wpabuf *msg)
{
	...
	if (wps->new_psk) {
		if (wps_cb_new_psk(wps->wps->registrar, wps->mac_addr_e, wps->new_psk, wps->new_psk_len)) {
...
enum wps_process_res wps_registrar_process_msg(struct wps_data *wps, enum wsc_op_code op_code, const struct wpabuf *msg)
{
	...
	case WSC_Done:
		...
		if (ret == WPS_FAILURE) {
			wps->state = SEND_WSC_NACK;
			wps_fail_event(wps->wps, WPS_WSC_DONE, wps->config_error, wps->error_indication);
		}
		return ret;
	default:
...
int wps_registrar_add_nfc_pw_token(struct wps_registrar *reg, const u8 *pubkey_hash, u16 pw_id, const u8 *dev_pw, size_t dev_pw_len)
{
	struct wps_nfc_pw_token *token;
...
int wps_registrar_add_nfc_password_token(struct wps_registrar *reg, const u8 *oob_dev_pw, size_t oob_dev_pw_len)
{
	...
	if (oob_dev_pw_len < WPS_OOB_PUBKEY_HASH_LEN + 2 + WPS_OOB_DEVICE_PASSWORD_MIN_LEN || oob_dev_pw_len > WPS_OOB_PUBKEY_HASH_LEN + 2 + WPS_OOB_DEVICE_PASSWORD_LEN) {
		return -1;
	}
	...
	return wps_registrar_add_nfc_pw_token(reg, hash, id, dev_pw, dev_pw_len);
}
