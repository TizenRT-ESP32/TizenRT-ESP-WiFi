...
int wps_build_public_key(struct wps_data *wps, struct wpabuf *msg, wps_key_mode_t mode)
{
	...
	if (mode != WPS_CALC_KEY_NO_CALC) {
		...
		} else if (wps->dev_pw_id >= 0x10 && wps->wps->ap && wps->dev_pw_id == wps->wps->ap_nfc_dev_pw_id) {
...
int wps_build_key_wrap_auth(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (wps_crypto_funcs.hmac_sha256) {
		wps_crypto_funcs.hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, wpabuf_head(msg), wpabuf_len(msg), hash);
	} else {
